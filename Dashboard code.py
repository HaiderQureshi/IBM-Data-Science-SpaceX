...
def update_piegraph(site_dropdown):
    if site_dropdown == 'ALL SITES':
        ...
        fig = px.pie(spacex_df, names='Launch Site', title=title_pie)
...
def build_scatter(site_dropdown,slider_range):
    ...
    if site_dropdown == 'ALL SITES':
        ...
        fig = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", color="Booster Version Category", title='Payload vs. Outcome for All Sites', hover_data=['Payload Mass (kg)'])
        
        return fig
    else:
        print(site_dropdown)
        filtered_df1= filtered_df[filtered_df['Launch Site'] == site_dropdown]
        fig = px.scatter(filtered_df1, x="Payload Mass (kg)", y="class", color="Booster Version Category", title=f'Payload vs. Outcome for {site_dropdown}', hover_data=['Payload Mass (kg)'])
        return fig
